...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'dateNaissance'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      dateNaissance: employee.dateNaissance?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      dateNaissance: restEmployee.dateNaissance ? dayjs(restEmployee.dateNaissance) : undefined,
    };
  }
